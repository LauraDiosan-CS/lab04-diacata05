...
/*
	...
	descr: get produse dupa data
	in: Data d, int numarProduse
	out: ref - toate produsele care au data respectiva
*/

Produs* Repo::getProduseDupaData(Data d, int &nP) {
	Produs* produseDupaData = new Produs[MAX_PRODUSE];
	for (int i = 0; i < this->numarProduse; i++) {
		if (this->produse[i].getData().equals(d)) {
			nP++;
			produseDupaData[nP] = this->produse[i];
		}
	}
	return produse;
}
